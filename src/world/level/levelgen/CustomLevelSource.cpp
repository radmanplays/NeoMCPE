...
const float CustomLevelSource::SNOW_CUTOFF = 0.5f;
const float CustomLevelSource::SNOW_SCALE = 0.3f;
static const int MAX_BUFFER_SIZE = 1024;

CustomLevelSource::CustomLevelSource(Level* level, long seed, int version, bool spawnMobs)
	:	random(seed),
	level(level),
...
/*public*/
void CustomLevelSource::prepareHeights(int xOffs, int zOffs, unsigned char* blocks, /*Biome*/void* biomes, float* temperatures) {
...
void CustomLevelSource::buildSurfaces(int xOffs, int zOffs, unsigned char* blocks, Biome** biomes) {
	int waterHeight = Level::DEPTH - 64;
...
/*private*/
float* CustomLevelSource::getHeights(float* buffer, int x, int y, int z, int xSize, int ySize, int zSize) {
	const int size = xSize * ySize * zSize;
	if (size > MAX_BUFFER_SIZE) {
		LOGI("CustomLevelSource::getHeights: TOO LARGE BUFFER REQUESTED: %d (max %d)\n", size, MAX_BUFFER_SIZE);
	}
...
std::string CustomLevelSource::gatherStats() {
	return "CustomLevelSource";
}
...
Biome::MobList CustomLevelSource::getMobsAt(const MobCategory& mobCategory, int x, int y, int z) {
